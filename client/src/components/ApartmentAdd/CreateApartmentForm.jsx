		...
		switch (field.name){
			...
			case "cata_apartment":
				...
				if(!/^[A-Za-z0-9 ,.'-]{3,20}$/.test(field.value)) {
					...
					else if(field.value.length > 20) {setHelperText({...helperText, cata_apartment: "Es muy largo"})}
					else{setHelperText({...helperText, cata_apartment: "Contiene caracteres no aceptados"})}
				}
				else{
			...
			case "mt2":
				...
				if(!/^[0-9 ,.']{2,10}$/.test(field.value)) {
					...
					else if(field.value.length > 10) {setHelperText({...helperText, mt2: "Es muy largo"})}
					else{setHelperText({...helperText, mt2: "Solo se permiten numeros, comas y puntos"})}
				}
				else{
			...
			case "number_apartment":
				...
				if(!/^[A-Z0-9-]{1,5}$/.test(field.value)) {
					...
					else{setHelperText({...helperText, number_apartment: "Solo se permiten mayusculas y numeros"})}
				}
				else{
    ...
    return (
        ...
        <div className= 'extContCAF'>
			...
			<Grid container direction="row" justify="space-around" alignItems="center" className={`componentDataBox ${classes.root}`} spacing={1}>
                ...
                <Grid>
					<Grid container spacing={1} alignItems="center" className={`${classes.last}`}>
                        <Grid item>
						<FormControl >
